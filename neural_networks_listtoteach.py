# -*- coding: utf-8 -*-
"""neural_networks_listtoteach.ipynb
...
df = pd.read_csv("/content/datas/list_to_teach.csv")
df.head(10)
...
history=model.fit(X_train, y_train, epochs=2000, batch_size=78,  verbose=0, validation_split=0.1)
...
dftest=pd.read_csv("/content/datas/list_to_predict.csv")
dftest['sigma'] = dftest['f1']*dftest['E']*3*10*(pi*197.3269805)**2
dftest.head()
...
def predictionsurbaseid(Z,A):
    ...
    plt.plot(abcisses, ynewfi,color="red", linewidth=1.5, linestyle="-")
...
def graph(Zbarre,Abarre):
    sigma=[]
    E=[]
    for j in range(len(dftest.loc[(dftest['Z']==Zbarre) & (dftest['A']==Abarre),:]['sigma'])):
        sigma.append(dftest.loc[(dftest['Z']==Zbarre) & (dftest['A']==Abarre),:]['sigma'].iloc[j])
        E.append(dftest.loc[(dftest['Z']==Zbarre) & (dftest['A']==Abarre),:]['E'].iloc[j])
...
plt.scatter(y_train,ynew)
plt.show()

Xnewbis=X_train
ynew= model.predict(Xnewbis)
...
bd = pd.read_csv("/content/datas/bdd_159nucleons.csv") #METTRE LA BASE DE DONNÉES AVEC UN FICHIER VALIDÉ PAR LE RÉSEAU DE NEURONE
bd['sigma'] = bd['f1']*bd['E']*3*10*(pi*197.3269805)**2
df = pd.read_csv("/content/datas/CartenoyauxZ1-26.csv",sep=';')


def pourunmemeZ(Z):
    Amin=int(df.loc[(df['Z']==Z)]['Amin'])
    Amax=int(df.loc[(df['Z']==Z)]['Astablemax'])
    listenoyaux=[]
    A=Amin
    while A<=Amax:
        listenoyaux.append((Z,A))
        A=A+1
    return(listenoyaux)
...
def creationbdd(Z,A):
    ...
    for j in range(len(X)):
        ...
        petite.append(Z)
        petite.append(A)
        petite.append(round(X[j][2],2))
        petite.append(ynewfi[j][0])
        petite.append('NN')
        valeurs.append(petite)
    dataNN= pd.DataFrame(valeurs, columns = ['Z','A','E','sigma','data sources'])
    return(dataNN)
    
def bddexpeouneuro(Z,A):
    if len(bd[(bd.Z == Z) & (bd.A == A)])==0:
        a=creationbdd(Z,A)
        return(a)
    else:
        liste2=[]
        for i in range(len(bd[(bd.Z == Z) & (bd.A == A)])):
          liste=[]
          liste.append(Z)
          liste.append(A)
          liste.append(bd.loc[(bd['Z']==Z) & (bd['A']==A),:]['E'].iloc[i])
          liste.append(bd.loc[(bd['Z']==Z) & (bd['A']==A),:]['sigma'].iloc[i])
          liste.append('EX')
          liste2.append(liste)
        dataexp= pd.DataFrame(liste2, columns = ['Z','A','E','sigma','data sources'])
        return(dataexp)

def basededonnéemixe(Zdebut,Zfin):
  listedonnées=[]
  listenoyaux=[]
  for i in range(Zfin-Zdebut+1):
    a=pourunmemeZ(Zdebut+i)
    for j in range(len(a)):
      listenoyaux.append(a[j])
  for k in range(len(listenoyaux)):
    listedonnées.append(bddexpeouneuro(listenoyaux[k][0],listenoyaux[k][1]))
  bdd = pd.concat(listedonnées, axis=0, ignore_index=True)
  return(bdd)

def createurlegende(Z,A):
  texte='A='
  Zbarre=str(A)
  txt=texte+Zbarre
  if basededonnéemixe(Z,Z).loc[(basededonnéemixe(Z,Z)['Z']==Z) & (basededonnéemixe(Z,Z)['A']==A),:]['data sources'].iloc[0]=='NN':
    final=txt+'  NN'
  if basededonnéemixe(Z,Z).loc[(basededonnéemixe(Z,Z)['Z']==Z) & (basededonnéemixe(Z,Z)['A']==A),:]['data sources'].iloc[0]=='EX':
    final=txt+'  EX'
  return(final)
...
liste=[]
Zàchoisir=22
for i in range(len(pourunmemeZ(Zàchoisir))):
  A=pourunmemeZ(Zàchoisir)[i][1]
  typedu=basededonnéemixe(Zàchoisir,Zàchoisir).loc[(basededonnéemixe(Zàchoisir,Zàchoisir)['Z']==Zàchoisir) & (basededonnéemixe(Zàchoisir,Zàchoisir)['A']==A),:]['data sources'].iloc[0]
  if typedu=='NN':
    plt.plot(basededonnéemixe(Zàchoisir,Zàchoisir).loc[(basededonnéemixe(Zàchoisir,Zàchoisir)['Z']==Zàchoisir) & (basededonnéemixe(Zàchoisir,Zàchoisir)['A']==A),:]['E'],basededonnéemixe(Zàchoisir,Zàchoisir).loc[(basededonnéemixe(Zàchoisir,Zàchoisir)['Z']==Zàchoisir) & (basededonnéemixe(Zàchoisir,Zàchoisir)['A']==A),:]['sigma'],linewidth=1)
  if typedu=='EX':
    plt.plot(basededonnéemixe(Zàchoisir,Zàchoisir).loc[(basededonnéemixe(Zàchoisir,Zàchoisir)['Z']==Zàchoisir) & (basededonnéemixe(Zàchoisir,Zàchoisir)['A']==A),:]['E'],basededonnéemixe(Zàchoisir,Zàchoisir).loc[(basededonnéemixe(Zàchoisir,Zàchoisir)['Z']==Zàchoisir) & (basededonnéemixe(Zàchoisir,Zàchoisir)['A']==A),:]['sigma'],'--')
  liste.append(createurlegende(Zàchoisir,A))

plt.legend(liste, loc='upper left')
plt.show()